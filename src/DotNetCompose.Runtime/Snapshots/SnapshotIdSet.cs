    ...
    public class SnapshotIdSet : IEnumerable<long>
    {
        ...
        public SnapshotIdSet Set(long id)
        {
            ...
            else if (offset >= 128)
            {
                if (!Get(id))
                {
                    ...
                    long[]? newBelowBound = _belowBound;
                    long targetLowerBound = SnapshotId.AlignToLowerBound(id);
            ...
            else
            {
                ...
                int location = Array.BinarySearch(_belowBound, id);
                if (location < 0)
                {
                    int insertLocation = ~location;
                    long[] newBelow = InsertIntoArray(_belowBound, insertLocation, id);
        ...
        public SnapshotIdSet And(SnapshotIdSet other)
        {
            ...
            if (other._lowerBound == _lowerBound && ReferenceEquals(other._belowBound, _belowBound))
            {
                ...
                long newLower = _lowerSet & other._lowerSet;
        ...
        {
            if (_belowBound != null)
                foreach (long id in _belowBound)
                    yield return id;
            if (_lowerSet != 0)
            {
                for (int i = 0; i < 64; i++)
        ...
        {
            SnapshotIdSet acc = initial;
            if (srcBelow != null)
                foreach (long id in srcBelow)
                    acc = op(acc, id);
            if (srcLower != 0)
                for (int i = 0; i < 64; i++)
                    if ((srcLower & (1L << i)) != 0)
